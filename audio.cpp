...
*/
	...
	// m_fillLevel.SetMinBufferSizeFrames(MsToFrames(200));
...
void cSoftHdAudio::ClockDriftCompensation()
{
	...
	if (m_packetCounter++ % 1000 == 0) {
		LOGDEBUG2(L_SOUND, "audio: %s: buffer fill level: %.1fms (target: %.1fms), clock drift compensation pitch: %.1fppm, PID controller: P=%.2fppm I=%.2fppm D=%.2fppm",
			__FUNCTION__,
			bufferFillLevelMs,

...
int64_t cSoftHdAudio::GetHardwareOutputPtsMs(void)
{
	...
	LOGINFO("hw delay: %ld samples: %ldms UsedBytes: %d input pts: %s", delaySamples, (int64_t)delaySamples * 1000 / m_hwSampleRate, m_pRingbuffer->UsedBytes(), Timestamp2String(PtsToMs(m_inputPts), 1));
...
void cSoftHdAudio::CyclicCall()
{
	...
	LOGINFO("Writing %d frames (%d bytes) to alsa device", framesToWrite, bytesToWrite);
